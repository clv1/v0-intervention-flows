...
export default function PhysicalStateWindow({ metrics, athletes }: { metrics: IPlayerLineChartData[], athletes: IAthlete[] }) {
  ...
  const calculateAverage = (data: any[], metricName: string): number | null => {
    ...
    if (metricName === 'strain') {
      validValues = validValues.map(value => value * (100 / 21));
    }
  ...
  const generateChartData = () => {
    ...
    if (!Array.isArray(periodString)) {
      const filteredMetrics = metrics?.filter(item => item.time_window === periodString) ?? [];
    ...
    for (let i = 0; i < 7; i++) {
      ...
      chartData.unshift({
        name: periodLabel,
        recovery: recoveryAvg !== null ? Math.round(recoveryAvg) : null,
        strain: strainAvg !== null ? Math.round(strainAvg) : null
      });
    }
  ...
  // let chartData: { name: string; recovery: number | null; strain: number | null; }[] = [];
  ...
  // chartData = filteredMetrics.map(item => ({
  //   name: item.label,
  //   recovery: item.recovery || null,
  //   strain: item.workload || null
  // }));
